...
class DeviceRepository:
    ...
    def create(self, barcode, device_type, brand, model, connector, is_engraved, status, place, now, user_id):
        with get_db() as db:
            db.execute("""INSERT INTO devices(internal_barcode,device_type,brand,model,connector,is_engraved,status,place,created_at,created_by)
                          VALUES(?,?,?,?,?,?,?,?,?,?)""",
                       (barcode, device_type, brand, model, connector, is_engraved, status, place, now, user_id))
            db.commit()
    ...
    def update(self, item_id, brand, model, connector, is_engraved, status, place, now, user_id):
        with get_db() as db:
            db.execute("""UPDATE devices SET brand=?,model=?,connector=?,is_engraved=?,status=?,place=?,updated_at=?,updated_by=?
                          WHERE id=?""",
                       (brand, model, connector, is_engraved, status, place, now, user_id, item_id))
            db.commit()
